(async()=>{
 ...
 function render(){
   ...
   box.querySelectorAll('.booking-request').forEach(card=>{
     ...
     card.querySelector('.availability-request').onclick=()=>{const raw=document.getElementById('availableSlots')?.value.trim();const hint=document.getElementById('availabilityHint');if(!raw){hint.textContent='اكتب المواعيد المتاحة أولًا.';return;}const number=wa(x.phone);if(number.length<7){hint.textContent='رقم العميل غير صالح للواتساب.';return;}const text=`مرحبًا ${x.name} 👋\n\nالمواعيد المتاحة لحجزك في مكتب Sergany:\n\n${raw}\n\nاختر الموعد المناسب لك وأرسل لي تأكيدك. شكرًا لك.`;window.open(`https://wa.me/${number}?text=${encodeURIComponent(text)}`,'_blank','noopener');hint.textContent='تم تجهيز رسالة واتساب بالمواعيد المتاحة.';};
   });
 }
