(async()=>{
 ...
 const token=sessionStorage.getItem('adminToken'); if(!token){location.href='admin.html';return;}
 ...
 function render(){
   ...
   const q=(document.getElementById('adminSearch')?.value||'').toLowerCase(),sf=document.getElementById('statusFilter')?.value||'all',df=document.getElementById('dateFilter')?.value||'all';
   const filtered=list.filter(x=>{const d=dateOf(x),ps=payment(x),text=`${x.name} ${x.phone} ${x.service}`.toLowerCase();if(q&&!text.includes(q))return false;if(sf!=='all'&&sf!==x.status&&sf!==ps)return false;if(df==='today'&&!same(d,now))return false;if(df==='yesterday'&&!same(d,y))return false;if(df==='month'&&!(d>=month&&d<=now))return false;return true});
