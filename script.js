...
const normalizeWhatsAppPhone=value=>{let s=String(value??'').replace(/[٠-٩]/g,d=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d))).trim();s=s.replace(/[^0-9+]/g,'');if(s.startsWith('+'))s=s.slice(1);else if(s.startsWith('00'))s=s.slice(2);else if(s.startsWith('0')&&s.length>=10&&s.length<=11)s='20'+s.slice(1);return s;};
...
const phoneInput=document.getElementById('phone');
  const normalizePhoneInput=value=>String(value??'').replace(/[٠-٩]/g,d=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[^0-9]/g,'').slice(0,15);
  if(phoneInput){
    const clean=()=>{phoneInput.value=normalizePhoneInput(phoneInput.value)};
    phoneInput.addEventListener('input',clean);
    phoneInput.addEventListener('paste',()=>setTimeout(clean,0));
    clean();
  }
  form.addEventListener('submit',e=>{e.preventDefault();const name=document.getElementById('name').value.trim(),phoneInput=document.getElementById('phone'),phone=normalizePhoneInput(phoneInput.value),service=document.getElementById('service').value,details=document.getElementById('details').value.trim();phoneInput.value=phone;if(!name||!phone||!service||!details)return;if(!/^[0-9]{7,15}$/.test(phone)){alert('رقم الموبايل يجب أن يحتوي على أرقام فقط، من 7 إلى 15 رقمًا.');phoneInput.focus();return;}fetch('/api/requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,phone,service,details})}).then(async r=>{if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d.error||'تعذر حفظ الطلب')} const text=`*طلب استشارة جديد – مكتب محمد السرجاني*%0A%0A👤 الاسم: ${encodeURIComponent(name)}%0A📱 الموبايل: ${encodeURIComponent(phone)}%0A🧾 الخدمة: ${encodeURIComponent(service)}%0A📝 التفاصيل: ${encodeURIComponent(details)}%0A%0A*يرجى التواصل مع العميل وتأكيد موعد الاستشارة.*`;window.open(`https://wa.me/${WA}?text=${text}`,'_blank');alert('تم حفظ طلبك وفتح واتساب لإرساله. انتظر الرد لتأكيد الاستشارة.');form.reset()}).catch(err=>alert(err.message))})}
function admin(){
